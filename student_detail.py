...
class Student:
    def __init__(self,root):
        ...
        # Automatically create the SQLite database and table if they don't exist
        # Use absolute path for the database file
        current_dir = os.path.dirname(os.path.abspath(__file__))
        self.db_path = os.path.join(current_dir, 'face_recognition.db')
        self.data_dir = os.path.join(current_dir, 'data')
        self.cascade_path = os.path.join(current_dir, 'Cascades', 'haarcascade_frontalface_default.xml')
        ...
        # img3=img3.resize((130,130),Image.Resampling.LANCZOS)   #High level img to Low level img
    ...
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_course.get()=="Select Course" or self.var_year.get()=="Select Year" or self.var_sem.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_std_email.get()=="" or self.var_gender.get()=="Select Gender" or self.var_dob.get()=="" or self.var_ForG.get()=="" or self.var_add.get()=="" or self.var_phone_no.get()=="" or self.var_roll.get()=="" or self.var_mentor.get()=="":
            ...
            try:
                conn=sqlite3.connect(self.db_path, timeout=60)
                my_cursor=conn.cursor()
                my_cursor.execute('insert into student_detail values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(
                    self.var_dep.get(),
                    self.var_course.get(),
                    self.var_year.get(),
                    self.var_sem.get(),
        ...
        if self.var_dep.get()=="Select Department" or self.var_course.get()=="Select Course" or self.var_year.get()=="Select Year" or self.var_sem.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_std_email.get()=="" or self.var_gender.get()=="Select Gender" or self.var_dob.get()=="" or self.var_ForG.get()=="" or self.var_add.get()=="" or self.var_phone_no.get()=="" or self.var_cgpa.get()=="" or self.var_mentor.get()=="":
            ...
            try:
                ...
                if(update>0):
                    conn=sqlite3.connect(self.db_path, timeout=60)
                    my_cursor=conn.cursor()
                    sql="""UPDATE student_detail SET dep= ?,course= ?,year= ?,semester= ?,name= ?,email= ?,gender= ?,phone= ?, address= ?,guardian=?, dob= ?,cgpa= ?,teacher=?,photosample= ?,eno=? where student_id= ?"""
                    val=(
                        self.var_dep.get(),
                        self.var_course.get(),
                        self.var_year.get(),
        ...
        if self.var_std_id.get()=="":
            ...
            try:
                ...
                if(delete>0):
                    conn=sqlite3.connect(self.db_path, timeout=60)
                    my_cursor=conn.cursor()
                    sql='delete from student_detail where student_id=?'
    ...
    def generate_dataset(self):
        if self.var_dep.get()=="Select Department" or self.var_course.get()=="Select Course" or self.var_year.get()=="Select Year" or self.var_sem.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_std_email.get()=="" or self.var_gender.get()=="Select Gender" or self.var_dob.get()=="" or self.var_ForG.get()=="" or self.var_add.get()=="" or self.var_phone_no.get()=="" or self.var_cgpa.get()=="" or self.var_mentor.get()=="" or self.var_roll.get()=="":
            ...
            try:
                ...
                # Use the actual student ID for image samples instead of a counter
                try:
                    current_id = int(self.var_std_id.get())
                except ValueError:
                    messagebox.showerror("Error", "Student ID must be a numeric value for photo samples", parent=self.root)
                    return
                ...
                while(True):
                    ...
                    for(x,y,w,h) in faces:
                        ...
                        #saving the captured face in the dataset folder TrainingImage using absolute path
                        img_filename = os.path.join(self.data_dir, f"image.{current_id}.{sampleNum}.jpg")
                ...
                if sampleNum > 0:
                    messagebox.showinfo("Result",f"Generating data set successfully ({sampleNum} images captured)", parent=self.root)
                else:
                    messagebox.showwarning("Warning", "No photos were captured! Ensure your face is visible and a blue rectangle appears around it.", parent=self.root)
